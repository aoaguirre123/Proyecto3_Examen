...
def registrarme(request):
    ...
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            mensaje = f'Tu cuenta de usuario: "{usuario.username}" ha sido creada con exito. Ya puedes ingresar a la tienda {premium}'
            messages.success(request, mensaje)
            return redirect(ingresar)
        else:
            messages.error(request, f'No se pudo crear la cuenta de usuario')
...
def misdatos(request):
    ...
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            messages.error(request, f'No se pudo actualizar la cuenta de usuario')
            show_form_errors(request, [form_usuario, form_perfil])
...
def boleta(request, nro_boleta):
    ...
    if Boleta.objects.filter(nro_boleta=nro_boleta).exists():
        ...
        if boleta_es_del_usuario or request.user.is_staff:
            ...
            messages.error(request, 'No tienes autorización para ver esta boleta')
            boleta = None
            return redirect(inicio)
    else:
        messages.error(request, f'La boleta solicitada no existe')
        return redirect(inicio)
...
def usuarios(request, accion, id):
    ...
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            perfil.usuario_id = usuario.id
            perfil.save()
            messages.success(request, f'El usuario {usuario.first_name} {usuario.last_name} fue guardado con exito.')
            return redirect(usuarios, 'actualizar', usuario.id)
        else:
            messages.error(request, f'No se pudo guardar el nuevo usuario')
            show_form_errors(request, [form_usuario, form_perfil])
...
def bodega(request):
    ...
    if request.method == 'POST':
        ...
        if cantidad == 1:
            ...
            messages.success(request, f'Se han ingresado {cantidad} nuevos "{producto.nombre}" a la bodega')
...
def enviar_correo_cambio_password(request, user, password):
    try:
        ...
        subject = 'Cambio de contraseña Playopolis'
...
def poblar(request):
    ...
    poblar_bd('ao.aguirre@playopolis.cl')
    return redirect(inicio)